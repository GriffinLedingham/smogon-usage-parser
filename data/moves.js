exports.BattleMovedex = {
  ...
  armthrust: {
    ...
    category: 'Physical',
    desc:
      "Hits two to five times. Has a 1/3 chance to hit two or three times, and a 1/6 chance to hit four or five times. If one of the hits breaks the target's substitute, it will take damage for the remaining hits. If the user has the Skill Link Ability, this move will always hit five times.",
  ...
  aromatherapy: {
    ...
    category: 'Status',
    desc:
      "Every Pokemon in the user's party is cured of its major status condition. Active Pokemon with the Sap Sipper Ability are not cured, unless they are the user.",
  ...
  aurawheel: {
    ...
    category: 'Physical',
    desc:
      'If the user is a Morpeko, this move will be Electric type. If the user is a Morpeko-Hangry, this move will be Dark type. Has a 100% chance to raise the users Speed by 1 stage. This move fails if the user is not a Morpeko or Morpeko-Hangry.',
    ...
    onModifyMove(move, pokemon) {
      if (
        pokemon.template.species === 'Morpeko-Hangry' &&
        move.type !== 'Dark'
      ) {
        move.type = 'Dark';
      } else if (
        pokemon.template.species === 'Morpeko' &&
        move.type !== 'Electric'
      ) {
        move.type = 'Electric';
      }
    ...
    effect: {
      ...
      onBeforeMove(pokemon, target, move) {
        if (this.effectData.duration === 1) {
          ...
          if (!target.isActive) {
            const possibleTarget = this.resolveTarget(
              pokemon,
              this.dex.getMove('pound'),
            );
            if (!possibleTarget) {
  ...
  bodypress: {
    ...
    category: 'Physical',
    desc:
      "Damage is calculated using the user's Defense stat, including stat stage changes. The user's Ability is used as normal.",
  ...
  breakingswipe: {
    ...
    category: 'Physical',
    desc:
      "100% chance to lower the foe's Attack by 1 stage. Hits all adjecent foes.",
  ...
  clangoroussoul: {
    ...
    category: 'Status',
    desc:
      "Raises the user's Attack, Defense, Special Attack, Special Defense, and Speed by 1 stage in exchange for the user losing 1/3 of its maximum HP, rounded down. Fails if the user would faint or if its Attack, Defense, Special Attack, Special Defense, and Speed stat stages are 6.",
    ...
    effect: {
      ...
      onAfterDamage(damage, target, source, effect) {
        if (
          ...
          this.getCategory(effect) === 'Physical'
        ) {
  ...
  courtchange: {
    ...
    category: 'Status',
    desc:
      "Switches the Spikes, Toxic Spikes, Stealth Rock, Sticky Web, Light Screen, Reflect, Aurora Veil, and Tailwind from the user's side to the target's side and vice versa.",
    ...
    onHitField(target, source) {
      ...
      for (let id of sideConditions) {
        let sourceLayers = side1.sideConditions[id]
          ? side1.sideConditions[id].layers || 1
          : 0;
        let targetLayers = side2.sideConditions[id]
          ? side2.sideConditions[id].layers || 1
          : 0;
      ...
      for (const targetCondition of removeTarget) {
        if (target.side.removeSideCondition(targetCondition)) {
          ...
          this.add(
            '-sideend',
            target.side,
            this.dex.getEffect(targetCondition).name,
            '[from] move: Defog',
            '[of] ' + source,
          );
          success = true;
      ...
      for (const sideCondition of removeAll) {
        if (source.side.removeSideCondition(sideCondition)) {
          this.add(
            '-sideend',
            source.side,
            this.dex.getEffect(sideCondition).name,
            '[from] move: Defog',
            '[of] ' + source,
          );
          success = true;
  ...
  doubleironbash: {
    ...
    category: 'Physical',
    desc:
      "Hits twice. If the first hit breaks the target's substitute, it will take damage for the second hit. Has a 30% chance to flinch the target.",
  ...
  dragondarts: {
    ...
    category: 'Physical',
    desc:
      'In singles, this move hits the target twice. In doubles, this move hits each target once.',
  ...
  entrainment: {
    ...
    onTryHit(target, source) {
      ...
      if (
        bannedTargetAbilities.includes(target.ability) ||
        bannedSourceAbilities.includes(source.ability) ||
    ...
    onHit(target, source) {
      ...
      if (oldAbility) {
        this.add(
          '-ability',
          target,
          this.dex.getAbility(target.ability).name,
  ...
  firepledge: {
    ...
    onPrepareHit(target, source, move) {
      for (const action of this.queue) {
        ...
        // @ts-ignore
        if (
          action.pokemon.side === source.side &&
      ...
      for (const ally of target.side.active) {
        if (ally && this.isAdjacent(target, ally)) {
          this.damage(
            ...
            ally,
            source,
            this.dex.getEffect('Flame Burst'),
          );
        }
      }
    },
  ...
  followme: {
    ...
    effect: {
      ...
      onFoeRedirectTarget(target, source, source2, move) {
        if (
          !this.effectData.target.isSkyDropped() &&
          this.validTarget(this.effectData.target, source, move.target)
        ) {
  ...
  gastroacid: {
    ...
    effect: {
      ...
      onStart(pokemon) {
        ...
        this.singleEvent(
          'End',
          this.dex.getAbility(pokemon.ability),
          pokemon.abilityData,
          pokemon,
          pokemon,
          'gastroacid',
  ...
  gmaxdrumsolo: {
    ...
    category: 'Physical',
    desc:
      "Power is equal to the base move's Max Move power. This move ignores the abilities of opposing Pokemon.",
          ...
          for (const moveSlot of source.moveSlots) {
            if (moveSlot.id === source.lastMove.id) {
              ...
              this.add(
                ...
                source,
                'move: Grudge',
                this.dex.getMove(source.lastMove.id).name,
              );
            }
          }
        }
  ...
  healbell: {
    ...
    category: 'Status',
    desc:
      "Every Pokemon in the user's party is cured of its major status condition. Active Pokemon with the Soundproof Ability are not cured.",
    shortDesc: "Cures the user's party of all status conditions.",
    id: 'healbell',
    isViable: true,
    name: 'Heal Bell',
  ...
  healblock: {
    ...
    effect: {
      ...
      onTryHeal(damage, target, source, effect) {
        if ((effect && effect.id === 'zpower') || this.effectData.isZ)
          return damage;
        return false;
      },
    },
  ...
  healingwish: {
    ...
    category: 'Status',
    desc:
      'The user faints and the Pokemon brought out to replace it has its HP fully restored along with having any major status condition cured. The new Pokemon is sent out at the end of the turn, and the healing happens before hazards take effect. Fails if the user is the last unfainted Pokemon in its party.',
  ...
  highjumpkick: {
    ...
    onMoveFail(target, source, move) {
      this.damage(
        ...
        source,
        source,
        this.dex.getEffect('High Jump Kick'),
      );
    },
    secondary: null,
    target: 'normal',
  ...
  icefang: {
    ...
    category: 'Physical',
    desc:
      'Has a 10% chance to freeze the target and a 10% chance to flinch it.',
  ...
  imprison: {
    ...
    effect: {
      ...
      onFoeBeforeMove(attacker, defender, move) {
        if (
          move.id !== 'struggle' &&
          this.effectData.source.hasMove(move.id) &&
          !move.isZ &&
          !move.isMax
        ) {
  ...
  instruct: {
    ...
    onHit(target, source) {
      ...
      this.runMove(
        target.lastMove.id,
        target,
        /** @type {number} */ (target.lastMoveTargetLoc),
      );
    },
    secondary: null,
    target: 'normal',
  ...
  leechseed: {
    ...
    effect: {
      ...
      onResidualOrder: 8,
      onResidual(pokemon) {
        let target = this.effectData.source.side.active[
          pokemon.volatiles['leechseed'].sourcePosition
        ];
        if (!target || target.fainted || target.hp <= 0) {
  ...
  lifedew: {
    ...
    category: 'Status',
    desc:
      "The user restores 1/4 of its maximum HP, rounded half up. If there is an adjacent ally, the user restores 1/4 of both its and its ally's maximum HP, rounded up.",
  ...
  lightscreen: {
    ...
    effect: {
      ...
      onAnyModifyDamage(damage, source, target, move) {
        if (
          target !== source &&
          target.side === this.effectData.target &&
          this.getCategory(move) === 'Special'
        ) {
          if (!target.getMoveHitData(move).crit && !move.infiltrates) {
  ...
  magikarpsrevenge: {
    ...
    category: 'Physical',
    desc:
      "Has a 100% chance to confuse the target and lower its Defense and Special Attack by 1 stage. The user recovers 1/2 the HP lost by the target, rounded half up. If Big Root is held by the user, the HP recovered is 1.3x normal, rounded half down. The user steals the foe's boosts. If this move is successful, the weather changes to rain unless it is already in effect, and the user gains the effects of Aqua Ring and Magic Coat.",
  ...
  metalburst: {
    ...
    category: 'Physical',
    desc:
      "Deals damage to the last opposing Pokemon to hit the user with an attack this turn equal to 1.5 times the HP lost by the user from that attack, rounded down. If the user did not lose HP from the attack, this move deals 1 HP of damage instead. If that opposing Pokemon's position is no longer in use and there is another opposing Pokemon on the field, the damage is done to it instead. Only the last hit of a multi-hit attack is counted. Fails if the user was not hit by an opposing Pokemon's attack this turn.",
  ...
  metronome: {
    ...
    category: 'Status',
    desc:
      "A random move is selected for use, other than After You, Assist, Baneful Bunker, Beak Blast, Belch, Bestow, Celebrate, Chatter, Copycat, Counter, Covet, Crafty Shield, Destiny Bond, Detect, Diamond Storm, Endure, Feint, Fleur Cannon, Focus Punch, Follow Me, Freeze Shock, Helping Hand, Hold Hands, Hyperspace Hole, Ice Burn, Instruct, King's Shield, Light of Ruin, Mat Block, Me First, Metronome, Mimic, Mind Blown, Mirror Coat, Mirror Move, Nature Power, Photon Geyser, Plasma Fists, Protect, Quash, Quick Guard, Rage Powder, Relic Song, Secret Sword, Shell Trap, Sketch, Sleep Talk, Snarl, Snatch, Snore, Spectral Thief, Spiky Shield, Spotlight, Steam Eruption, Struggle, Switcheroo, Techno Blast, Thief, Thousand Arrows, Thousand Waves, Transform, Trick, V-create, or Wide Guard.",
    ...
    effect: {
      ...
      onAfterDamage(damage, target, source, effect) {
        if (
          ...
          effect.effectType === 'Move' &&
          source.side !== target.side &&
          this.getCategory(effect) === 'Special'
        ) {
          this.effectData.position = source.position;
          this.effectData.damage = 2 * damage;
        }
  ...
  obstruct: {
    ...
    category: 'Status',
    desc:
      "The user is protected from attacks made by other Pokemon during this turn, and Pokemon trying to make contact with the user have their Defense lowered by 2 stages. This move has a 1/X chance of being successful, where X starts at 1 and triples each time this move is successfully used. X resets to 1 if this move fails, if the user's last move used is not Baneful Bunker, Detect, Endure, King's Shield, Protect, Quick Guard, Spiky Shield, or Wide Guard, or if it was one of those moves and the user's protection was broken. Fails if the user moves last this turn.",
  ...
  psychoshift: {
    ...
    category: 'Status',
    desc:
      "The user's major status condition is transferred to the target, and the user is then cured. Fails if the user has no major status condition or if the target already has one.",
  ...
  purify: {
    ...
    category: 'Status',
    desc:
      'The target is cured if it has a major status condition. If the target was cured, the user restores 1/2 of its maximum HP, rounded half up.',
  ...
  pursuit: {
    ...
    category: 'Physical',
    desc:
      "If an opposing Pokemon switches out this turn, this move hits that Pokemon before it leaves the field, even if it was not the original target. If the user moves after an opponent using Parting Shot, U-turn, or Volt Switch, but not Baton Pass, it will hit that opponent before it leaves the field. Power doubles and no accuracy check is done if the user hits an opponent switching out, and the user's turn is over; if an opponent faints from this, the replacement Pokemon does not become active until the end of the turn.",
        ...
        for (const condition of sideConditions) {
          if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
            this.add(
              '-sideend',
              pokemon.side,
              this.dex.getEffect(condition).name,
              '[from] move: Rapid Spin',
              '[of] ' + pokemon,
            );
          }
  ...
  reflecttype: {
    ...
    onHit(target, source) {
      if (
        source.template &&
        (source.template.num === 493 || source.template.num === 773)
      )
        return false;
  ...
  relicsong: {
    ...
    onHit(target, pokemon, move) {
      if (
        pokemon.baseTemplate.baseSpecies === 'Meloetta' &&
        !pokemon.transformed
      ) {
    ...
    onHit(target, source) {
      ...
      if (oldAbility) {
        this.add(
          '-ability',
          source,
          this.dex.getAbility(source.ability).name,
          '[from] move: Role Play',
          '[of] ' + target,
        );
        return;
  ...
  safeguard: {
    ...
    category: 'Status',
    desc:
      "For 5 turns, the user and its party members cannot have major status conditions or confusion inflicted on them by other Pokemon. It is removed from the user's side if the user or an ally is successfully hit by Defog. Fails if the effect is already active on the user's side.",
  ...
  skillswap: {
    ...
    onTryHit(target, source) {
      ...
      if (
        bannedAbilities.includes(target.ability) ||
        bannedAbilities.includes(source.ability)
      ) {
        return false;
      }
    },
    onHit(target, source, move) {
      ...
      if (targetAbility.id !== sourceAbility.id) {
        ...
        source.abilityData = {
          id: toID(source.ability),
          target: source,
        };
        target.abilityData = {
          id: toID(target.ability),
          target: target,
        };
      }
  ...
  skyattack: {
    ...
    category: 'Physical',
    desc:
      'Has a 30% chance to flinch the target and a higher chance for a critical hit. This attack charges on the first turn and executes on the second. If the user is holding a Power Herb, the move completes in one turn.',
    ...
    effect: {
      ...
      onAnyDragOut(pokemon) {
        if (
          ...
          pokemon === this.effectData.source
        )
          return false;
      },
  ...
  sparklyswirl: {
    ...
    category: 'Special',
    desc:
      "Every Pokemon in the user's party is cured of its major status condition.",
    shortDesc: "Cures the user's party of all status conditions.",
    id: 'sparklyswirl',
    isNonstandard: 'LGPE',
    isViable: true,
  ...
  stealthrock: {
    ...
    effect: {
      ...
      onSwitchIn(pokemon) {
        if (pokemon.hasItem('heavydutyboots')) return;
        let typeMod = this.dex.clampIntRange(
  ...
  steamroller: {
    ...
    category: 'Physical',
    desc:
      'Has a 30% chance to flinch the target. Damage doubles and no accuracy check is done if the target has used Minimize while active.',
    shortDesc: '30% chance to flinch the target.',
    id: 'steamroller',
    isNonstandard: 'Past',
    name: 'Steamroller',
  ...
  stockpile: {
    ...
    effect: {
      ...
      onEnd(target) {
        ...
        if (
          this.effectData.def !== this.effectData.layers * -1 ||
          this.effectData.spd !== this.effectData.layers * -1
        ) {
          this.hint(
  ...
  suckerpunch: {
    ...
    onTry(source, target) {
      ...
      if (
        !action ||
        action.choice !== 'move' ||
        (action.move.category === 'Status' && action.move.id !== 'mefirst') ||
  ...
  tarshot: {
    ...
    category: 'Status',
    desc:
      "Lowers the target's Speed by 1 stage. Until the target switches out, its effectiveness against Fire type moves is changed.",
  ...
  teatime: {
    ...
    category: 'Status',
    desc:
      'Forces all active Pokemon to consume their held berries. This move bypasses Substitutes.',
  ...
  thunderfang: {
    ...
    category: 'Physical',
    desc:
      'Has a 10% chance to paralyze the target and a 10% chance to flinch it.',
  ...
  twister: {
    ...
    category: 'Special',
    desc:
      'Has a 20% chance to flinch the target. Power doubles if the target is using Bounce, Fly, or Sky Drop, or is under the effect of Sky Drop.',
  ...
  waterpledge: {
    ...
    onPrepareHit(target, source, move) {
      for (const action of this.queue) {
        ...
        if (
          action.pokemon.side === source.side &&
          ['firepledge', 'grasspledge'].includes(action.move.id)
        ) {
          // @ts-ignore
          this.prioritizeAction(action);
          this.add('-waiting', source, action.pokemon);
  ...
  watershuriken: {
    ...
    basePowerCallback(pokemon, target, move) {
      if (
        pokemon.template.species === 'Greninja-Ash' &&
  ...
  weatherball: {
    ...
    category: 'Special',
    desc:
      "Power doubles if a weather condition other than Delta Stream is active, and this move's type changes to match. Ice type during Hail, Water type during Primordial Sea or Rain Dance, Rock type during Sandstorm, and Fire type during Desolate Land or Sunny Day.",
        ...
        if (target && !target.fainted) {
          ...
          if (damage)
            this.add(
              ...
              target.getHealth,
              '[from] move: Wish',
              '[wisher] ' + this.effectData.source.name,
            );
        }
      },
    },
  ...
  yawn: {
    ...
    category: 'Status',
    desc:
      'Causes the target to fall asleep at the end of the next turn. Fails when used if the target cannot fall asleep or if it already has a major status condition. At the end of the next turn, if the target is still active, does not have a major status condition, and can fall asleep, it falls asleep. If the target becomes affected, this effect cannot be prevented by Safeguard or a substitute, or by falling asleep and waking up during the effect.',
  ...
  // ISLE OF ARMOR PLACEHOLDERS JUNE 15
  ...
  burningjealousy: {
    ...
    category: 'Special',
    desc:
      'Burns all Pokemon on the field that have boosted a stat during the same turn.',
  ...
  expandingforce: {
    ...
    category: 'Special',
    desc:
      "If the current active terrain is Psychic Terrain, this move's base power is boosted, and this move hits all opposing Pokemon.",
  ...
  grassyglide: {
    ...
    category: 'Physical',
    desc:
      'If this move is used while Grassy Terrain is active, its user will nearly always move first.',
  ...
  surgingstrikes: {
    ...
    category: 'Physical',
    desc:
      'This move is always a critical hit unless the target is under the effect of Lucky Chant or has the Battle Armor or Shell Armor Abilities. This move hits the target three times.',
  ...
  lashout: {
    ...
    category: 'Physical',
    desc:
      "This move's base power is doubles if one of its stats were lowered this turn.",
  ...
  mistyexplosion: {
    ...
    desc:
      "The user faints after using this move, even if this move fails for having no target. This move is prevented from executing if any active Pokemon has the Damp Ability. If Misty Terrain is active, this move's power is doubled.",
  ...
  risingvoltage: {
    ...
    category: 'Special',
    desc:
      "If the current active terrain is Electric Terrain, this move's base power is doubled.",
  ...
  steelroller: {
    ...
    category: 'Physical',
    desc:
      'Ends the effects of Electric Terrain, Grassy Terrain, Misty Terrain, and Psychic Terrain. This move fails if there is not a terrain active.',
  ...
  terrainpulse: {
    ...
    category: 'Special',
    desc:
      "Power doubles if a terrain is active, and this move's type changes to match. Electric type during Electric Terrain, Grass type during Grassy Terrain, Fairy type during Misty Terrain, and Psychic type during Psychic Terrain.",
  ...
  tripleaxel: {
    ...
    category: 'Physical',
    desc:
      "Hits three times. If one of the hits breaks the target's substitute, it will take damage for the remaining hits. If the user has the Skill Link Ability, this move will always hit three times.",
  ...
  shellsidearm: {
    ...
    category: 'Special',
    desc:
      "This move becomes a physical attack if the target's Defense is lower than its Special Defense, including stat stage changes. This move has a 20% chance to poison the target.",
