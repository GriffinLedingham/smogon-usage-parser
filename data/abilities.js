...
let BattleAbilities = {
    ...
    onResidual(pokemon) {
      if (
        ...
        if (
          pokemon.hp &&
          !pokemon.item &&
          this.dex.getItem(pokemon.lastItem).isBerry
        ) {
  ...
  mimicry: {
    shortDesc:
      "This Pokemon's type changes to match the Terrain. Type reverts when Terrain ends.",
    onStart(pokemon) {
      ...
      if (move.flags['contact'] && source.ability !== 'mummy') {
        ...
        if (oldAbility) {
          this.add(
            ...
            target,
            'ability: Mummy',
            this.dex.getAbility(oldAbility).name,
            '[of] ' + source,
          );
        }
      }
  ...
  perishbody: {
    desc:
      'Making contact with this Pokemon starts the Perish Song effect for it and the attacker. This effect does not happen if this Pokemon already has a perish count.',
    shortDesc:
  ...
  powerconstruct: {
    ...
    onResidual(pokemon) {
      ...
      let newMaxHP = pokemon.volatiles['dynamax']
        ? 2 * pokemon.baseMaxhp
        : pokemon.baseMaxhp;
  ...
  sandspit: {
    desc:
      'When this Pokemon is hit by an attack, Sandstorm begins. This effect happens after the effects of Max and G-Max Moves.',
  ...
  stickyhold: {
    ...
    onTakeItem(item, pokemon, source) {
      if (
        this.suppressingAttackEvents(pokemon) ||
        !pokemon.hp ||
  ...
  wanderingspirit: {
    ...
    onDamagingHit(damage, target, source, move) {
      ...
      if (
        ['illusion', 'wanderingspirit', 'wonderguard'].includes(source.ability)
      )
        return;
      if (move.flags['contact']) {
        ...
        if (target.side === source.side) {
          ...
          this.add(
            ...
            this.dex.getAbility(sourceAbility).name,
            'Wandering Spirit',
            '[of] ' + source,
          );
        }
  ...
  zenmode: {
    ...
    onEnd(pokemon) {
      ...
      if (
        ...
        pokemon.formeChange(
          /** @type {string} */ (pokemon.species.battleOnly),
          this.effect,
          false,
          '[silent]',
        );
